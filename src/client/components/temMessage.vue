      ...
      <div class="tmsg-commentshow">
        <ul class="tmsg-commentlist">
          <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
            <article class="tmsg-article">
              <header>
                <img
                  :src="item.avatar?item.avatar:'static/img/tou.jpg'"
                  :onerror="$store.state.errorImg"
                />
                <div class="i-name">{{item.username? item.username :'游客'}}</div>
                <div class="i-class">{{item.label ? item.label: '暗中观察'}}</div>
                <div class="i-time">
                  <time>{{item.createTime | parseTime('{y}-{m}-{d}')}}</time>
                </div>
              </header>
              <section>
                <p class="tmsg-article" v-html="analyzeEmoji(item.textarea)">{{analyzeEmoji(item.textarea)}}</p>
                <div
                  v-if="haslogin"
                  class="tmsg-replay"
            ...
            <ul v-show="item.ChildsSon" class="tmsg-commentlist" style="padding-left:60px;">
              <li
                class="tmsg-c-item"
                v-for="(citem,cindex) in item.ChildsSon"
                :key="'citem'+cindex"
              >
                <article class>
                  <header>
...
export default {
  ...
  methods: {
    ...
    showCommentList: function(initData) {
      ...
      //公用设置数据方法
      function setData(result, isInitData) {
        if (isInitData) {
          that.commentList = result.data.data;
        } else {
          that.commentList = that.commentList.concat(result.data.data);
        }
        // alert(result.data.allNum)
        if (that.commentList.length < result.data.allNum) {
      ...
      this.$store
        .dispatch("getCommet", { pageIndex: this.pageIndex })
        .then(res => {
